...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
    ...
    BTreeNode* curr = children_[idx];
    int tmp = elements_[idx-1];
    std::vector<int>::iterator pos = std::find(elements_.begin(), elements_.end(), tmp);
    curr->elements_.insert(curr->elements_.begin(), tmp);
    elements_.erase(pos);
    int toinsert = prev->elements_.back();
    prev->elements_.pop_back();
    elements_.insert(pos, toinsert);
    /*
    BTreeNode* curr = children_[idx];
    int tmp = elements_[idx-1];
    int toinsert = prev->elements_.back();
    ...
    std::vector<int>::iterator pos = std::find(elements_.begin(), elements_.end(), tmp);
    curr->elements_.insert(curr->elements_.begin(), tmp);
    elements_.emplace(pos, toinsert);
    elements_.erase(--pos);
    */
