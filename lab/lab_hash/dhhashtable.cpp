...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     size_t first_hash = hashes::hash(key, size);
     size_t second_hash = hashes::secondary_hash(key, size);
     int i = 0;
     while (should_probe[(first_hash + i * second_hash) % size]) {
       i++;
     }
     std::pair<K, V>* p = new std::pair<K, V>(key, value);
     table[(first_hash + i * second_hash) % size] = p;
     should_probe[(first_hash + i * second_hash) % size] = true;
