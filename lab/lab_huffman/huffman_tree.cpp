...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for (unsigned i = 0; i < frequencies.size(); i++) {
       tmp = new TreeNode(frequencies[i]);
       singleQueue.push(tmp);
     }

     while (singleQueue.size() + mergeQueue.size() > 1) {
       tmp = removeSmallest(singleQueue, mergeQueue);
       tmp2 = removeSmallest(singleQueue, mergeQueue);
       tmp3 = new TreeNode(tmp->freq.getFrequency() + tmp2->freq.getFrequency());
       tmp3->left = tmp;
       tmp3->right = tmp2;
       mergeQueue.push(tmp3);
     }
