...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (findw1 == dict.end() || findw2 == dict.end()) return false;
    if (findw1->second.size() != findw2->second.size()) return false;
    for (unsigned i = 0; i < findw1->second.size(); i++) {
      if (findw1->second[i] != findw2->second[i]) {
        return false;
      }
    }
    return true;
