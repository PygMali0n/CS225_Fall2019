...
class KDTree
{
  ...
  private:
     ...
     */
     void construct(vector<Point<Dim>>& newPoints, int left, int right, int dimension, KDTreeNode*& node);
     ...
     void find(Point<Dim>& query, Point<Dim>& currentBest, KDTreeNode* node, int dimension) const;
};
